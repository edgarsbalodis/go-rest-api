...
func (d *Database) PostComment(ctx context.Context, cmt comment.Comment) (comment.Comment, error) {
	...
	if err != nil {
		return comment.Comment{}, fmt.Errorf("failes to insert comment: %w", err)
	}
...
func (d *Database) UpdateComment(ctx context.Context, id string, cmt comment.Comment) (comment.Comment, error) {
	...
	if err != nil {
		return comment.Comment{}, fmt.Errorf("fauled to update comment: %w", err)
	}

	if err := rows.Close(); err != nil {
		return comment.Comment{}, fmt.Errorf("failes to close rows: %w", err)
	}
